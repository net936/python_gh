  ...
  <div>
    ...
    <section class="cart-page flex-view">
      <div class="left-flex">
        ...
        <a-select style="width: 100%; margin-top: 16px" placeholder="请选择就诊时间" allowClear v-model:value="pageData.receiverTime">
          <a-select-option :value="item" v-for="(item, index) in pageData.zzsjRange" :key="index">{{ item }}</a-select-option>
        </a-select>
      ...
      <div class="right-flex">
        ...
        <div class="price-view">
          <div class="price-item flex-view">
            <div class="item-name">费用总价</div>
            <div class="price-txt">¥{{ pageData.amount }}</div>
          </div>
          <div class="price-item flex-view">
            <div class="item-name">费用优惠</div>
            <div class="price-txt">¥0</div>
          </div>
          <div class="price-item flex-view">
            <div class="item-name">费用折扣</div>
            <div class="price-txt">¥0</div>
          </div>
...
<script setup>
  ...
  onMounted(() => {
    ...
    pageData.zzsjRange = route.query.zzsj.split(',');
    let leftWeekDayArr = getLeftWeekDate();
    // 在诊时间与剩余日期的交集，比如['周一','周六']与['周六','周日']的交集是['周六']
    pageData.zzsjRange = pageData.zzsjRange.filter(function(v){ return leftWeekDayArr.indexOf(v) > -1 })
  });

  // 获取本周剩余
  function getLeftWeekDate() {
    var now = new Date();
    var day = now.getDay();
    ...
    if (week === '周一') {
      ...
      return ['周二', '周三', '周四', '周五', '周六', '周日'];
    } else if (week === '周三') {
      return ['周三', '周四', '周五', '周六', '周日'];
    } else if (week === '周四') {
      return ['周四', '周五', '周六', '周日'];
    } else if (week === '周五') {
      return ['周五', '周六', '周日'];
